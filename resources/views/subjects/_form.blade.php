{{-- Shared fields for subjects.create / subjects.edit / subjects.request-edit.
     ...
     blocks the request before it ever reaches SubjectController's server-
...
<div class="row">
    ...
    <div class="col-md-4 mb-3">
        ...
        <input type="text" name="subject_code" class="form-control" value="{{ old('subject_code', $subject->subject_code ?? '') }}" placeholder="e.g. COMP 016">
    </div>
    <div class="col-md-4 mb-3">
        <label class="form-label">Title</label>
        <input type="text" name="title" class="form-control" value="{{ old('title', $subject->title ?? '') }}">
    </div>
</div>
    ...
    <div class="col-md-3 mb-3">
        ...
        <select name="year_level" class="form-select">
            ...
            @foreach ([1 => '1st Year', 2 => '2nd Year', 3 => '3rd Year', 4 => '4th Year'] as $value => $label)
                <option value="{{ $value }}" @selected((string) old('year_level', $subject->year_level ?? '') === (string) $value)>{{ $label }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-3 mb-3">
        ...
        <select name="semester" class="form-select">
            ...
            @foreach (['1st', '2nd', 'summer'] as $sem)
                <option value="{{ $sem }}" @selected(old('semester', $subject->semester ?? null) === $sem)>{{ ucfirst($sem) }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-3 mb-3">
